...
## Expansion: `else if` support and Bug Fixes

- [x] Fix: Wrap `cfg` metadata in `all()` to support multiple entries in `else` branches.
- [x] Implement recursive `cfg_iif!` macro to support `else if` chains.
    - [x] Support standard `#[cfg(...)]` syntax in chains.
    - [x] Support shorthand syntax in chains.
- [x] Add unit tests for `else if` chains (standard and shorthand).
- [x] Update crate-level documentation in `src/lib.rs` with `else if` examples.
- [x] Update `CHANGELOG.md` with the bug fix and new feature.

## Verification

- [x] Verify `no_std` compatibility by building for a bare-metal target (`thumbv7m-none-eabi`).
